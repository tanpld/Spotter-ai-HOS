...
class HOSTestCase(TestCase):
    ...
    def setUp(self):
        ...
        patcher_dist = patch(
            "hos_planner.views.road_distance_miles", side_effect=_mock_road_distance
        )
...
class LongTripTest(HOSTestCase):
    ...
    def test_rest_break_inserted(self):
        ...
        all_statuses = {
            entry["status"] for day in self.data["days"] for entry in day["log_entries"]
        }
        self.assertTrue(
            rest_statuses & all_statuses, "No rest break found in multi-day trip"
        )
    ...
    def test_fueling_stop_injected(self):
        ...
        self.assertGreater(
            len(fueling_entries), 0, "No fueling stop found for >1,000-mile trip"
        )
